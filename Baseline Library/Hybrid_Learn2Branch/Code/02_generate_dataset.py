...
def make_samples(in_queue, out_queue,n_samples):
    """
...
def collect_samples(instances, outdir, rng, n_samples, n_jobs, time_limit):
    ...
    for i in range(n_jobs):
        p = mp.Process(
                target=make_samples,
                args=(orders_queue, answers_queue,n_samples),
                daemon=True)
        workers.append(p)
        p.start()
...
if __name__ == "__main__":
    ...
    if args.problem == 'cauctions':
        ...
        out_dir = f'{basedir}/{args.problem}'
